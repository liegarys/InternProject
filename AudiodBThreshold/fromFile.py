...
def process_wav_file_in_chunks(file_path, chunk_size, threshold, output_folder):
    # Open the WAV file
    with wave.open(file_path, 'r') as wav_file:
        ...
        # Show the plot
        #plt.show()
        print(f"Cropped time : {crossing_times[-1] - crossing_times[0]}")
        ...
        output_file_path = os.path.join(output_folder, "cropped_" + os.path.basename(file_path))
...
def process_folders(base_path, folders, chunk_size, threshold, output_base_path):
    for folder in folders:
        folder_path = os.path.join(base_path, folder)
        output_folder = os.path.join(output_base_path, folder)
        for file_path in glob(os.path.join(folder_path, '*.wav')):
            process_wav_file_in_chunks(file_path, chunk_size, threshold, output_folder)

# Example usage
file_path = "D:\\Python Course\\ses_kayitlari\\Evet\\aliEvet2.wav"

base_path = "D:\\Python Course\\ses_kayitlari"
folders = ["landim"]

output_base_path = "D:\\Python Course\\ses_kayitlari\\processed_files"

chunk_size = 1024  # Number of frames per chunk
threshold = 30  # dB threshold

process_folders(base_path, folders, chunk_size, threshold, output_base_path)